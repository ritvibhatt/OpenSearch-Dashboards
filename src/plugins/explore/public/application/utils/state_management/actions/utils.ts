...
type SQLBucketFormat = 'datetime' | 'month' | 'year';

/**
 * Build the SQL expression that buckets the time field for a histogram, plus
 * the format of its (string) output so the result can be parsed back.
 *
 * Uses the native `date_histogram()` bucket function
 * (opensearch-project/sql#5514), which lowers to native (numeric) bucketing on
 * the Analytics Engine — dramatically faster at scale than a per-row string
 * expression (≈ on par with PPL's `span()`; see
 * docs/plugins/explore/sql-histogram-performance.md). It accepts the standard
 * interval expression (e.g. `30m`, `12h`, `30d`, `1M`, `1y`) and always returns
 * a uniform `'YYYY-MM-DD HH:mm:ss'` string, so the format is always `'datetime'`.
 *
 * It must be used as a subquery-aliased GROUP BY key (direct `GROUP BY
 * date_histogram(...)` is not supported by the engine planner).
 */
const intervalToSQLBucket = (
  interval: string,
  timeFieldName: string
): { expr: string; format: SQLBucketFormat } => {
  ...
  return {
    expr: `date_histogram('field'=${field}, 'interval'='${normalized}')`,
    format: 'datetime',
  };
};

/**
 * Parse a DATE_FORMAT bucket string (UTC) back to epoch milliseconds for the
 * chart. `format` must match the one used to build the bucket expression.
 */
const parseSQLBucketToMs = (bucket: string, format: SQLBucketFormat): number => {
  if (format === 'year') {
    // "2026"
    return Date.UTC(Number(bucket), 0, 1);
  }
  if (format === 'month') {
    // "2026-05"
...
export const processRawResultsForHistogram = (
  ...
  const { aggs, breakdownField, finalInterval, timeFieldName } = histogramConfig;
  ...
  if (isSQLHistogram) {
    ...
    // The bucket column is a DATE_FORMAT string; parse it back to epoch ms
    // using the same format the query was built with.
    const { format } = intervalToSQLBucket(finalInterval!, timeFieldName!);
    ...
    if (breakdownField) {
      ...
      rawResults.hits.hits.forEach((hit) => {
        ...
        const timestamp = parseSQLBucketToMs(String(source[bucketName]), format);
    ...
    Object.entries(aggs as Record<number, any>).forEach(([key, value]) => {
      ...
      if (aggTypeKey === 'date_histogram') {
        const buckets = rawResults.hits.hits.map((hit) => {
          ...
          const timeBucket = parseSQLBucketToMs(String(source[bucketName]), format);
