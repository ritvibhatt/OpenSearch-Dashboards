...
describe('Utils - Histogram Breakdown Support', () => {
  ...
  describe('processRawResultsForHistogram (SQL)', () => {
    it('parses datetime bucket strings back to epoch ms', () => {
      ...
      const result = utils.processRawResultsForHistogram(
        'SELECT ...',
        rawResults,
        config,
        true
    ...
    it('parses month bucket strings (date_histogram datetime output)', () => {
      const rawResults: any = {
        hits: {
    ...
    it('builds per-breakdown-value series when breakdownField is set', () => {
      ...
      const result: any = utils.processRawResultsForHistogram('SELECT ...', rawResults, config, true);
      ...
      const series200 = result.breakdownSeries.series.find(
        (s: any) => s.breakdownValue === '200'
      );
      expect(series200.dataPoints).toEqual([
        [Date.UTC(2023, 0, 1, 0), 10],
        [Date.UTC(2023, 0, 1, 1), 5],
      ]);
      const series500 = result.breakdownSeries.series.find(
        (s: any) => s.breakdownValue === '500'
      );
